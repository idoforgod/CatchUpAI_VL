...
이 프롬프트는 매일 학습을 시작할 때 사용합니다.
Roadmap이 이미 준비되어 있는 상태에서 사용하세요.

**사용 절차**:
1. 아래 [1단계]의 "학습 진행 상황" 섹션만 현재 상태에 맞게 업데이트
2. 이 파일 전체를 AI에게 전달
...
**Roadmap 파일 경로**:
```
vl_roadmap/20260205_RoadMap_Remotion-VideoCreation.md
```
...
> **매 세션마다 아래 3개 항목을 업데이트하세요**

**현재 진행 중인 모듈**:
```
[예: M2 - Remotion Core 기초]
```

**가장 최근 WorkLog 파일**:
```
[예: vl_worklog/20260205_M1_Remotion-VideoCreation.md]
```

**이전 세션의 "Tomorrow's focus"** (있다면):
```
[이전 WorkLog의 Tomorrow's focus 내용을 붙여넣기]
```
...
**WorkLog**:
- `vl_worklog/YYYYMMDD_M{X}_Remotion-VideoCreation.md`
...
**파일 경로**: `vl_worklog/YYYYMMDD_M{X}_Remotion-VideoCreation.md`
**예시**: `vl_worklog/20260206_M2_Remotion-VideoCreation.md`
...
**환경**:
- [ ] 개발 환경 실행 가능 (VS Code, Terminal, Node.js)
- [ ] Remotion 프로젝트 접근 가능 (my-first-video/)
- [ ] 인터넷 연결 (필요 시)
